...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(JSON.stringify(self.chain[self.height]) == JSON.stringify(block))
                ...
                reject(Error("Add block was not a success"));
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           ...
           reject(Error("Block with " + hash + " not found"));
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(stars.length > 0)
                ...
                reject(Error("No stars found for the " + address));
        }); 
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i <= self.chain.length; i++) {
                ...
                let flag = await block.validate();
                if (!flag)
                    if (block.previousBlockHash != self.chain[i - 1].hash)
                    errorLog.push('Block ' + block.height + ' doesnt match with its previous block');
                else 
